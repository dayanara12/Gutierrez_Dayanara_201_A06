...
[https://github.com/dayanara12/Gutierrez_Dayanara_201_A06.git](https://github.com/dayanara12/Gutierrez_Dayanara_201_A06.git)
...
![(/Users/dayanaragutierrez/Desktop/Assignment6.png)](/Users/dayanaragutierrez/Desktop/Assignment6.png)
...
for (player in names (player_info)) {
  player_info [[player]]$height <- player_heights [player]
}
...
for (player in names (player_info)) {
  ...
  points_per_game_list [[substr (player, 1, 2)]] <- points_per_game
}
...
rounded_points <- sapply (points_per_game_list, function (x) round (x, 2))
...
my_round <- function (x) {
  ifelse (nchar (x) == 2, round (x, -1),
          ifelse (nchar (x) == 3, round (x, -2),
                  ifelse (nchar (x) == 4, round (x, -3), x)))
}

rounded_matrix <- apply (mat_rand, MARGIN = c(1, 2), FUN = my_round)
...
average_temps <- max_temps %>%
  group_by (year) %>%
  summarize (across (jan:dec, mean, na.rm = TRUE))
...
colorado_temp <- max_temps %>%
  filter (state == "CO")

ohio_temp <- max_temps %>%
  filter (state == "OH")

virginia_temp <- max_temps %>%
  filter (state == "VA")
...
colorado_median <- colorado_temp %>%
  group_by (year) %>%
  summarize (median_temp = median(jan:dec, na.rm = TRUE))


max_year <- colorado_median %>%
  filter(median_temp == max(median_temp)) %>%
  pull(year)

min_year <- colorado_median %>%
  filter(median_temp == min(median_temp)) %>%
  pull(year)

print (paste("The maximum median annual temperature of", max(colorado_median$median_temp), "occurred in", max_year, "and the minimum median annual temperature of", min(colorado_median$median_temp), "occurred in", min_year,"."))
